        ...
        {!collapsed && <>
          ...
          {inputMode === "manual" && timerState === "idle" && (
            <div style={{ backgroundColor:C.bg, borderRadius:10, padding:"12px 14px", marginBottom:12 }}>
              ...
              <div style={{ display:"flex", gap:8, alignItems:"center" }}>
                <input type="number" step="0.1" min="0" max="24" value={inputVal}
                  onChange={e => setInputVal(e.target.value)} placeholder="例) 1.5 (h)" style={{ ...inp, flex:1 }} />
                <span style={{ fontSize:13, color:C.muted }}>h</span>
                <button onClick={handleManualSave} disabled={saving || !inputVal}
