...
import { C, fmt, occ, dow, hourly, FREE_LIMIT, loadS, saveS, getClosingPeriod, calcTake } from "../lib/constants";
    ...
    <>
      ...
      {showDetail && (
        <div style={{ position:"fixed", inset:0, backgroundColor:"#00000088", zIndex:200, display:"flex", alignItems:"flex-end" }} onClick={dismiss}>
          <div onClick={e=>e.stopPropagation()} style={{ backgroundColor:C.surface, borderRadius:"20px 20px 0 0", width:"100%", maxWidth:480, margin:"0 auto", maxHeight:"80vh", overflowY:"auto", padding:24, paddingBottom:40, position:"relative" }}>
            <button onClick={dismiss} style={{ position:"absolute", top:14, right:16, background:"none", border:"none", fontSize:28, color:C.muted, cursor:"pointer", lineHeight:1, padding:"8px" }}>×</button>
...
function SalesChart({ reports }) {
  ...
  const netOf = r => r.net_sales || Math.round(r.gross_sales / 1.1);
  const maxSales = Math.max(...recent.map(r => netOf(r)));
  const chartH = 80, barW = 32, gap = 8, topPad = 20;
  const totalW = recent.length * (barW + gap) - gap;
  return (
    <Card style={{ padding:"14px 14px 10px", marginBottom:14 }}>
      <div style={{ fontSize:12, color:C.muted, fontWeight:700, marginBottom:12 }}>直近{recent.length}日の売上（税抜）</div>
      <div style={{ overflowX:"auto" }}>
        <svg width={Math.max(totalW, 280)} height={chartH + topPad + 42} style={{ display:"block", margin:"0 auto" }}>
          {recent.map((r, i) => {
            const net = netOf(r);
            const barH = Math.max(4, Math.round((net / maxSales) * chartH));
            const x = i * (barW + gap), y = topPad + chartH - barH;
            const isToday = r.date === new Date().toISOString().slice(0, 10);
            const barColor = net >= 60000 ? C.green : net >= 53000 ? C.accentLight : C.orange;
            return (
              <g key={r.id}>
                <rect x={x} y={y} width={barW} height={barH} rx={4} fill={barColor} opacity={isToday ? 1 : 0.75}/>
                <text x={x+barW/2} y={y-4} textAnchor="middle" fontSize={9} fill={C.sub}>{Math.round(net/1000)}k</text>
                <text x={x+barW/2} y={topPad+chartH+16} textAnchor="middle" fontSize={9} fill={isToday?C.accentLight:C.muted} fontWeight={isToday?"bold":"normal"}>{r.date.slice(5).replace("-","/")}</text>
                <text x={x+barW/2} y={topPad+chartH+28} textAnchor="middle" fontSize={8} fill={isToday?C.accentLight:C.muted}>{["日","月","火","水","木","金","土"][new Date(r.date).getDay()]}</text>
              </g>
            );
          })}
        </svg>
      ...
      <div style={{ display:"flex", gap:12, marginTop:6, flexWrap:"wrap" }}>
        {[{color:C.green,label:"60k以上"},{color:C.accentLight,label:"53〜60k"},{color:C.orange,label:"53k未満"}].map(({color,label})=>(
...
// ━━━ タクロー応援メッセージ ━━━━━━━━━━━━━━━━━━
function TakuroMessage({ hasTarget, achievement, remainingShifts, remainingAmount, neededPerShift, periodEnd, todayStr }) {
  if (!hasTarget) return null;

  const isLastDay   = periodEnd === todayStr;
  const isAchieved  = achievement >= 100;
  const isClose     = achievement >= 90 && !isAchieved;
  const isSprintEnd = remainingShifts <= 3 && !isAchieved && remainingAmount > 0;
  const isJustOver  = remainingShifts === 0 && !isAchieved;
  ...
  if (isAchieved) {
    msg = "今月の目標達成！本当にお疲れさまでした 🎉";
    color = C.green;
    bg = C.green + "18";
  } else if (isLastDay) {
    msg = `今日が締め日です。あと ${fmt(remainingAmount)} 円、全力で！`;
    color = C.orange;
    bg = C.orange + "18";
  } else if (isJustOver) {
    msg = "今月はここまで。記録が積み重なるほど来月に活きます 💪";
    color = C.muted;
    bg = C.card;
  } else if (isSprintEnd) {
    msg = `残り ${remainingShifts} 出番！1本 ${fmt(neededPerShift)} 円で達成できます`;
    color = C.gold;
    bg = C.gold + "18";
  } else if (isClose) {
    msg = `あと ${fmt(remainingAmount)} 円。もう少しです！`;
    color = C.accentLight;
    bg = C.accentGlow;
  }
  ...
  return (
    <div style={{ display:"flex", alignItems:"center", gap:10,
      ...
      <div style={{ fontSize:13, fontWeight:600, color, lineHeight:1.6 }}>{msg}</div>
    </div>
  );
}
...
function AnalysisTodayCard({ reports }) {
  ...
  const landingForecast = avgPerRide > 0 && avgRideCount > 0
    ? (todaySalesNet > 0 ? todaySalesNet + remainingRides * avgPerRide : avgPerRide * avgRideCount)
    : 0;
  ...
  return (
    <div style={{ marginBottom:14 }}>
      ...
      {show && (
        <div style={{ backgroundColor:C.accentGlow, border:`1px solid ${C.accentLight}33`, borderRadius:12, padding:"14px 16px" }}>
          <div style={{ fontSize:10, color:C.muted, marginBottom:4 }}>着地予想（税抜き）</div>
          {landingForecast > 0 ? (
            <div style={{ display:"flex", alignItems:"baseline", justifyContent:"space-between", gap:8 }}>
              <div style={{ fontSize:26, fontWeight:900, color:C.accentLight }}>
                {fmt(landingForecast)}<span style={{ fontSize:12, marginLeft:2 }}>円</span>
              </div>
              <div style={{ fontSize:10, color:C.muted, textAlign:"right", lineHeight:1.6 }}>
                {todaySalesNet > 0
                  ? `${fmt(todaySalesNet)}円＋残り${remainingRides}回×${fmt(avgPerRide)}`
                  : `平均単価${fmt(avgPerRide)}×${avgRideCount}回ペース`}
              </div>
            </div>
          ) : (
            <div style={{ fontSize:13, color:C.muted, fontWeight:600 }}>データ不足（日報5件以上で表示）</div>
          )}
        </div>
      )}
    </div>
...
export default function Dashboard({ reports, user, onOpenReport, onManageArea, rankPrefs = { showMyRank:false, showTopSales:false }, appMode = "standard", onGoShift, onUpdateReport, onGoRanking, onUpdateUser }) {
  ...
  const monthTotalNet = monthReports.reduce((s,r) => s + (r.net_sales || Math.round((r.gross_sales || 0) / 1.1)), 0); // 税抜合計
  const monthTarget   = parseInt(user.target) || 0;  // 0 = 未設定（税抜）
  const hasTarget     = monthTarget > 0;
  const achievement   = hasTarget ? Math.round((monthTotalNet / monthTarget) * 100) : 0;
  const avgSales      = monthReports.length ? Math.round(monthTotal / monthReports.length) : 0;
  const avgSalesNet   = monthReports.length ? Math.round(monthTotalNet / monthReports.length) : 0; // 税抜平均売上
  ...
  const takePay       = loadS("taxi_takepay", { rate:55, deduction:30000, expenses:0 });
  const isKoTaku      = user?.workType === "個人タクシー";
  const estimatedTake = calcTake(monthTotal, takePay, user?.workType);
  ...
  const remainingAmount = Math.max(0, monthTarget - monthTotalNet); // 税抜同士で比較
      ...
      <div style={{ maxWidth:600, margin:"0 auto", padding: isSimpleLarge ? "16px 10px 100px" : "16px 16px 100px", zoom: isSimpleLarge ? 1.32 : 1 }}>
        ...
        <Card style={{ marginBottom:14, padding:"24px 20px", borderColor:C.gold+"33" }}>
          ...
          <div style={{ fontSize:52, fontWeight:900, color:C.text, lineHeight:1.1 }}>
            {fmt(monthTotalNet)}<span style={{ fontSize:20, color:C.muted, marginLeft:6 }}>円</span>
          </div>
          ...
          <div style={{ marginTop:16, padding:"14px 0 0", borderTop:`1px solid ${C.border}` }}>
            ...
            <div style={{ fontSize:34, fontWeight:900, color:achColor }}>
              {monthTotalNet >= monthTarget
                ? <span style={{ color:C.green }}>達成！🎉</span>
                : <>{fmt(monthTarget - monthTotalNet)}<span style={{ fontSize:16, marginLeft:4 }}>円</span></>
              }
            </div>
            {hasTarget && monthTotalNet < monthTarget && remainingShifts > 0 && (
  ...
  return (
    <div style={{ maxWidth:600, margin:"0 auto", padding:"16px 16px 100px" }}>
      ...
      <Card style={{ marginBottom:14, borderColor:C.gold+"33" }}>
        <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-start", marginBottom:12 }}>
          <div>
            ...
            <div style={{ fontSize:32, fontWeight:900, color:C.text }}>
              {fmt(monthTotalNet)}<span style={{ fontSize:13, color:C.muted, marginLeft:4 }}>円</span>
            </div>
          ...
          <div style={{ textAlign:"right" }}>
            ...
            <div style={{ fontSize:18, fontWeight:800, color:achColor }}>
              {monthTotalNet >= monthTarget
                ? <span style={{ color:C.green }}>達成！🎉</span>
                : `${fmt(monthTarget - monthTotalNet)}円`}
            </div>
          </div>
        </div>
        {hasTarget && <>
        ...
        {hasTarget && monthTotalNet >= monthTarget && (
          ...
          <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", marginTop:12, paddingTop:12, borderTop:`1px solid ${C.border}` }}>
            <div>
              <div style={{ fontSize:11, color:C.muted }}>
                {isKoTaku ? "推定手取り（売上−経費）" : `推定手取り（歩合${takePay.rate}%）`}
              </div>
            ...
            <div style={{ fontSize:10, color:C.muted, textAlign:"right" }}>
              <div>{isKoTaku ? `経費 ${fmt(takePay.expenses||0)}円` : `控除 ${fmt(takePay.deduction)}円`}</div>
      ...
      <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:10, marginBottom:14 }}>
        <KpiCard label="平均売上（税抜）" value={fmt(avgSalesNet)} unit="円" accent={C.accentLight} />
