...
function stripChome(addr) {
  return (addr || "").replace(/\s*[0-9０-９一二三四五六七八九十百]+丁目.*$/, "").trim();
}
  ...
  const areaStats = useMemo(() => {
    ...
    rawRides
      ...
      .forEach(({ address, amount }) => {
        ...
        const fallback = stripChome(address) || "不明";
        const key   = resolved ? resolved.label : fallback;
        const ward  = resolved?.ward  || "";
        const area  = resolved?.area  || fallback;
