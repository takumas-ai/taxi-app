...
// 未マッチ住所の丁目以下を除去（例: "新宿3丁目2-1" → "新宿"）
function stripChome(addr) {
  return (addr || "")
    .replace(/\s*[0-9０-９一二三四五六七八九十百]+丁目.*$/, "")
    .replace(/\s*[0-9]+[-－][0-9].*$/, "")
    .trim();
}
...
export default function MapScreen({ reports, user }) {
  ...
  const [selectedArea, setSelectedArea] = useState(null); // #63 エリアクリック詳細
  ...
  const areaStats = useMemo(() => {
    ...
    rawRides
      ...
      .forEach(({ address, amount, date, hour }) => {
        const resolved = resolveArea(address);
        const stripped = stripChome(address) || "不明";
        let ward = resolved?.ward || "";
        let area = resolved?.area || stripped;
        if (!resolved && stripped !== "不明") {
          // 区・市名を分離して小さく表示できるよう分解
          const m = stripped.match(/^(.*?[都道府県市区])/);
          if (m && m[1].length < stripped.length) {
            ward = m[1];
            area = stripped.slice(m[1].length).trim() || stripped;
          }
        }
        const key = resolved ? resolved.label : (ward + area);
        if (!map[key]) map[key] = { count: 0, total: 0, ward, area, rides: [] };
        map[key].count++;
        map[key].total += Number(amount) || 0;
        map[key].rides.push({ amount, date, hour });
      });
    return Object.entries(map)
      .map(([label, { count, total, ward, area, rides }]) => ({
        label, ward, area, count, rides,
  ...
  return (
    <div style={{ maxWidth:480, margin:"0 auto", padding:"16px 16px 100px" }}>
      ...
      <div style={{ display:"flex", gap:6, marginBottom:14 }}>
        {[["highFare","💰 高単価"],["area","📊 エリア"],["point","📍 スポット"]].map(([v, lbl]) => (
      ...
      <div style={{ backgroundColor:C.surface, borderRadius:12, marginBottom:14, overflow:"hidden" }}>
        <div onClick={() => setFiltersOpen(p => !p)}
          ...
          <span style={{ fontSize:13, fontWeight:700, color:C.text }}>
            🔍 絞り込み
            {hasFilter && (
              <span style={{ marginLeft:6, fontSize:11, color:C.accentLight, fontWeight:600 }}>
                {[
                  timeSlotIdx > 0 ? TIME_SLOTS[timeSlotIdx].label : null,
      ...
      {!loading && tab === "area" && (
        <div>
          ...
          {areaStats.length === 0 ? (
            ...
            return (
              <div key={i} onClick={() => setSelectedArea(s)}
                style={{ backgroundColor:C.surface, borderRadius:12, padding:"14px 16px",
                ...
                <div>
                  ...
                  <div style={{ fontSize:15, fontWeight:700, color:C.text }}>{s.area}</div>
                  <div style={{ fontSize:12, color:C.muted, marginTop:2 }}>{s.count}回 · タップで詳細</div>
                </div>
        ...
        <div>
          {bizPoints.length === 0 ? (
            <div style={{ textAlign:"center", padding:48, color:C.muted, fontSize:13 }}>
              ...
              マイスポットが未登録です<br/>
              <span style={{ fontSize:11 }}>高単価タブの「スポットへ登録」から追加できます</span>
            </div>
          ) : (
            <>
              <div style={{ fontSize:12, color:C.muted, marginBottom:10 }}>
                登録スポット {bizPoints.length}件
                {hasFilter && " （絞り込み適用中）"}
              </div>
              {pointStats.map((p, i) => {
      ...
      {/* ── エリア詳細モーダル（#63） ── */}
      {selectedArea && (() => {
        const rides = [...(selectedArea.rides || [])].sort((a, b) => b.amount - a.amount);
        return (
          <div style={{ position:"fixed", inset:0, backgroundColor:"#00000090", zIndex:300, display:"flex", alignItems:"flex-end" }}
            onClick={() => setSelectedArea(null)}>
            <div onClick={e => e.stopPropagation()}
              style={{ backgroundColor:C.surface, borderRadius:"20px 20px 0 0", width:"100%",
                maxWidth:480, margin:"0 auto", padding:"20px 20px 48px", maxHeight:"75vh", display:"flex", flexDirection:"column" }}>
              ...
              <div style={{ fontSize:16, fontWeight:800, marginBottom:4, color:C.text }}>
                {selectedArea.ward && <span style={{ fontSize:12, color:C.muted, marginRight:6 }}>{selectedArea.ward}</span>}
                {selectedArea.area}
              </div>
              <div style={{ fontSize:12, color:C.muted, marginBottom:14 }}>{rides.length}件 · 平均¥{fmt(selectedArea.avg)}</div>
              <div style={{ overflowY:"auto", flex:1 }}>
                {rides.map((r, i) => (
                  <div key={i} style={{ display:"flex", justifyContent:"space-between", alignItems:"center",
                    padding:"10px 0", borderBottom:`1px solid ${C.border}44` }}>
                    <div style={{ fontSize:12, color:C.muted }}>
                      {r.date && <span>{r.date}({dowLabel(r.date)})</span>}
                      {r.hour !== null && r.hour !== undefined && <span style={{ marginLeft:6 }}>{r.hour}時台</span>}
                    </div>
                    <div style={{ fontSize:16, fontWeight:700, color:r.amount>=5000?C.green:r.amount>=3000?C.gold:C.text }}>
                      ¥{fmt(r.amount)}
                    </div>
                  </div>
                ))}
